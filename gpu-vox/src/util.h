...
struct voxinfo {
	...
	void align_self(){
        ...
        glm::vec3 centres;
        centres.x =(bbox.max.x + bbox.min.x) / 2;
        centres.y = (bbox.max.y + bbox.min.y) / 2;
...
//        Now the scales that ought to be used
	...
	void print() {
		...
		fprintf(stdout, "[Voxelization] Translation : x: %f y: %f z: %f\n", translation.x, translation.y, translation.z);
		fprintf(stdout, "[Voxelization] scale : x: %f y: %f z: %f\n", scales.x, scales.y, scales.z);
	}

    void align_vertices(glm::vec3 &vertex){
        vertex.x = vertex.x + translation.x;
        vertex.y = vertex.y + translation.y;
        vertex.z = vertex.z + translation.z;
//    Now the scaling part
        vertex.x = vertex.x * scales.x;
        vertex.y = vertex.y * scales.y;
        vertex.z = vertex.z * scales.z;
//  Currently things are in a unit cube with center at origin. Hence, now we can go ahead and make a translation
        vertex.x = vertex.x + 0.5f;
        vertex.y = vertex.y + 0.5f;
        vertex.z = vertex.z + 0.5f;
//  Now our elements are between 0 and 1. Hence, we can scale it finally to the required grid size
        vertex.x = vertex.x * gridsize.x;
        vertex.y = vertex.y * gridsize.y;
